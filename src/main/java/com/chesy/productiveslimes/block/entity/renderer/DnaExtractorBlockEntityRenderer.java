...
public class DnaExtractorBlockEntityRenderer implements BlockEntityRenderer<DnaExtractorBlockEntity> {
    ...
    public void render(DnaExtractorBlockEntity pBlockEntity, float tickDelta, MatrixStack pPoseStack, VertexConsumerProvider pBufferSource, int light, int overlay) {
        ...
        itemRenderer.renderItem(itemStack, ModelTransformationMode.FIXED, getLightLevel(pBlockEntity.getWorld(), pBlockEntity.getPos()), OverlayTexture.DEFAULT_UV, pPoseStack, pBufferSource, pBlockEntity.getWorld(), 1);

        pPoseStack.pop();
    }
    private int getLightLevel(World level, BlockPos blockPos) {
        int bLight = level.getLightLevel(LightType.BLOCK, blockPos);
        int sLight = level.getLightLevel(LightType.SKY, blockPos);
